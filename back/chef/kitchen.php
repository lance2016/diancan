...
require("../db.func.php");
$pdo=getConnection();
$sql="select * from user where finish=0 order by c_time";
$rs=select($sql,$pdo);
$sql2="select count(*) from user where finish=0";
$to=select($sql2,$pdo);
?>
<div style="width:100%;text-align:center;height:60px;background-color:#352F31;position:fixed">
	<br>
	<span style="color:white;font-size:20px;"><b>厨房管理</b></span>
</div>
    ...
    <button class="btn btn-info pull-right" onclick="history.go(0)">刷新</button>
     ...
     <?php for($i=0;$i<$to[0][0];$i++){?>
        <table style="width:100%;border:solid 1px #A69E9C;margin-bottom:30px;"> 
			<tr height="100px" style="font-size:25px;text-align:left">  <!--显示座位号-->	
            	<td style="padding-left:30px; width:40%;">桌号：</td>
                <?php echo "<td id=".$rs[$i]['desk_num'].">".$rs[$i]['desk_num'];?></td>
				<?php echo"<td><button class='btn btn-success' onclick='finish(this.id)' style='height:40px;width:80%;'  id=".$rs[$i]['desk_num'].">完成</button></td>";?>
            </tr>
            <tr height="20px"  style="font-size:15px;">		 <!--显示表单信息-->
                ...
                <td style="border-bottom:dotted 1px #000000">状态</td>		
            </tr>
         <?php 
...
function finish(id){
         $.post('../edit.php',{'name':'finish','zhuohao':id},function(data){
